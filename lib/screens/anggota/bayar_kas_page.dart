...
import '../../image_helper.dart'; // Import Helper yang baru dibuat
...
class _BayarKasPageState extends State<BayarKasPage> {
  ...
  // MENGGUNAKAN HELPER KOMPRESI
  Future<void> _pickImage() async {
    // Tampilkan loading sebentar saat proses kompresi
    String? compressedBase64 = await ImageHelper.pickAndCompress();
  ...
  void _kirimPembayaran() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        String docId = "${user!.uid}_${_bulanController.text}";
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Pembayaran terkirim!"), backgroundColor: Colors.indigo, behavior: SnackBarBehavior.floating),
        );
      } catch (e) {
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildInputField(controller: _bulanController, label: "Untuk Pembayaran Bulan", hint: "Contoh: Januari 2024", icon: Icons.calendar_month_rounded),
                  const SizedBox(height: 15),
                  _buildInputField(controller: _jumlahController, label: "Nominal Bayar", hint: "10000", icon: Icons.payments_rounded, isNumber: true, prefix: "Rp "),
                  const SizedBox(height: 25),
          ...
          child: Column(
            children: [
              Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text(isExpired ? "WAKTU PEMBAYARAN HABIS" : "SISA WAKTU PEMBAYARAN", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: isExpired ? Colors.red : Colors.green)),
              ]),
              const SizedBox(height: 10),
              Text(isExpired ? "Segera hubungi bendahara" : "${sisa.inDays} Hari ${sisa.inHours % 24}j ${sisa.inMinutes % 60}m ${sisa.inSeconds % 60}s", style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color(0xFF1A237E))),
            ],
          ),
        );
      },
  ...
  Widget _buildUploadBox() {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(25), border: Border.all(color: Colors.grey.withOpacity(0.2))),
        ...
        child: _base64Image == null 
          ...
          : Stack(fit: StackFit.expand, children: [
              ...
              Positioned(right: 10, top: 10, child: CircleAvatar(backgroundColor: Colors.black54, child: IconButton(icon: const Icon(Icons.refresh, color: Colors.white), onPressed: _pickImage))),
            ]),
      ),
    );
  }

  Widget _buildInputField({required TextEditingController controller, required String label, required String hint, required IconData icon, bool isNumber = false, String? prefix}) {
    return Container(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          labelText: label, hintText: hint, prefixText: prefix, prefixIcon: Icon(icon, color: const Color(0xFF1A237E)),
  ...
  Widget _buildHorizontalLoading() {
    return Column(children: [
      ...
      const SizedBox(height: 10),
      ClipRRect(borderRadius: BorderRadius.circular(10), child: const LinearProgressIndicator(minHeight: 6, backgroundColor: Colors.white, valueColor: AlwaysStoppedAnimation<Color>(Color(0xFF1A237E)))),
    ]);
  }
  ...
  Widget _buildSubmitButton() {
    return SizedBox(width: double.infinity, height: 60,
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1A237E), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)), elevation: 5),
